...
public:
										...
										/*out*/IWbemClassObject ** ppErrorObject)
		{
			...
			{				
				...
				(*ppErrorObject)->Put(lpwstrDescription, 0, &var, 0);
				if (FAILED(hr))
				{
					return hr;
				}
			...
			if (bstrOperation != NULL)
			{
				...
				(*ppErrorObject)->Put(lpwstrOperation, 0, &var, 0);
				if (FAILED(hr))
				{
					return hr;
				}
			...
			if (bstrParameter != NULL)
			{
				...
				(*ppErrorObject)->Put(lpwstrParameterInfo, 0, &var, 0);
				if (FAILED(hr))
				{
					return hr;
				}
			...
			if (bstrProviderName != NULL)
			{
				...
				(*ppErrorObject)->Put(lpwstrProviderName, 0, &var, 0);
				if (FAILED(hr))
				{
					return hr;
				}
...
public:
	...
	{	
		...
		hr = pPath->GetInfo(0L, &ulPathInfo);
		if(FAILED(hr))
		{
			return hr;
		}
...
class CImpersonateClientHelper
{
	private:
		BOOL m_bImpersonate;	
		BOOL m_bImpersonateSupported;
	public:
		CImpersonateClientHelper()
		{ 
			m_bImpersonate = FALSE;
			IsImpersonateSupported();
		}
		...
		HRESULT GetCurrentImpersonationLevel ( DWORD & a_Level )
		{
			...
			{
				// setting it to impersonate on win9x platform so that
				// the calling function in the provider code does not
...
private:
		void IsImpersonateSupported()
		{
			OSVERSIONINFO os;
			m_bImpersonateSupported = FALSE;
			os.dwOSVersionInfoSize = sizeof(OSVERSIONINFO);
			if(GetVersionEx(&os))
			{
				if(os.dwMajorVersion > 4 ||									
					(os.dwMajorVersion == 4 && os.dwPlatformId == VER_PLATFORM_WIN32_NT)) // NT 4
				{
					m_bImpersonateSupported = TRUE;
				}
			}
		}
